...
from fcnn import value_policy_approximator as model_fn
...
def expand_states(states, env):
    ...
    @returns rewards (Array): A numpy array of shape (N, num_acts, 1) containing the
                              respective rewards.
    """
    children, rewards = list(zip(*(env.expand_state(s) for s in states)))
...
@jax.jit
def l2_regularizer(params, reg=1e-3):
...
def update(i, opt_state, batch):
    ...
    loss, grads = loss_frads_fun(params, batch)
...
def train(rng, env, batch_size=128, num_epochs=5, num_iterations=21,
    ...
    for e in range(num_epochs):
        ...
        for it in range(num_iterations):
            ...
            # Make targets for the generated episodes using the most
            # recent params and build a batch generator.
            params = get_params(opt_state)
            tgt_vals, tgt_acts = make_targets(children, rewards, params)
            data = {"X" : states, "v" : tgt_vals, "pi": tgt_acts, "w" : w}
            rng, sub_rng = jax.random.split(rng)
